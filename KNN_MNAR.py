...
def  load_data(dataset, miss_rate, imputer_name, setting = 'node'):
    ...
    if setting == 'node':
        total_n = X.shape[0]
        n_u = math.ceil(total_n * miss_rate)
        rand = np.random.RandomState(0)  # Fixed random output
        print("X = {}, n_u = {}".format(X.shape[0], n_u))
        unknow_set = rand.choice(list(range(0, X.shape[0])), n_u, replace=False)

    train_slice = np.ones(np.shape(test_set))
    train_slice[:, unknow_set] = 0
    test_slice = 1 - train_slice  #(True if the value is missing).
    ...
    MAE, RMSE = get_error(imputer, X=test_set, test_slice = test_slice , h=h)
    ...
    for dataset in datasets:
        for j in impute_str:
            for i in range(6):
                ...
                MAE, RMSE = load_data(dataset, miss_rate = miss_rate,imputer_name = j, setting = 'node')
                print(dataset, 'node' , "{} rate = {}, MAE ={:.6}, RMSE ={:.6}".format(j,miss_rate,MAE, RMSE))
